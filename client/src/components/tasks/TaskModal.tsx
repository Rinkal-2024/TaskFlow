...
export const TaskModal = ({ isOpen, onClose, onSuccess, task, mode }: TaskModalProps) => {
  ...
  // Fetch users for assignee dropdown (only for admins)
  const { data: usersResponse } = useQuery({
    queryKey: ['users'],
    queryFn: () => apiService.getUsers({}, { page: 1, limit: 100 }),
    enabled: user?.role === 'admin', // Only fetch users if user is admin
  })
  ...
  useEffect(() => {
    if (mode === 'edit' && task) {
      ...
      // For new tasks, always start with "todo" status
      setFormData({
        title: '',
        description: '',
        status: 'todo' as TaskStatus, // New tasks always start as "todo"
        priority: 'medium' as TaskPriority,
        dueDate: '',
        tags: [],
        // For members, automatically assign to themselves when creating
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For members, always assign to themselves
    if (user?.role === 'member') {
      submitData.assigneeId = user._id
    } else {
      // For admins, only include assigneeId if it's not empty
