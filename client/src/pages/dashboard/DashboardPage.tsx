...
export const DashboardPage = () => {
  ...
  const getStatCards = () => {
    if (user?.role === 'admin') {
      return [
        ...
        {
          title: 'Total Users',
          value: systemStats?.data?.totalUsers || 0,
          icon: Users,
        ...
        },
        {
          ...
          value: systemStats?.data?.activeSessions || 0,
          icon: Activity,
  ...
  const priorityStats = [
    {
      label: 'Urgent',
      value: taskStats.tasksByPriority?.urgent || 0,
      color: 'bg-red-500',
      textColor: 'text-red-600'
    },
    {
      label: 'High',
      value: taskStats.tasksByPriority?.high || 0,
      color: 'bg-orange-500',
      textColor: 'text-orange-600'
    },
    {
      label: 'Medium',
      value: taskStats.tasksByPriority?.medium || 0,
      color: 'bg-yellow-500',
      textColor: 'text-yellow-600'
    },
    {
      label: 'Low',
      value: taskStats.tasksByPriority?.low || 0,
      color: 'bg-green-500',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {user?.role === 'admin' ? (
          <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
            ...
            ) : systemStats?.data?.recentActivity?.length ? (
              <div className="space-y-3">
                {systemStats.data.recentActivity.slice(0, 5).map((activity: any, index: number) => (
                  <div key={index} className="flex items-start space-x-3">
                    <div className="flex-shrink-0 w-2 h-2 bg-blue-500 rounded-full mt-2"></div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-gray-900">
                        {activity.action} by <span className="font-medium">{activity.user?.firstName}</span>
                      </p>
