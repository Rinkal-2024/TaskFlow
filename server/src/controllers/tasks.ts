...
export const getTaskById = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (req.user.role === UserRole.MEMBER) {
      ...
      //   throw new AppError(
      //     "Access denied. You can only view tasks you created or are assigned to.",
      //     403,
      //   );
      // }
    }
...
export const createTask = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { title, description, status, priority, dueDate, tags, assigneeId } =
      req.body;
    ...
    await ActivityLog.create({
      ...
      changes: { title, assignee: assigneeUser?.email || "Unassigned" },
    });
...
export const updateTask = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (req.user.role === UserRole.MEMBER && updates.assigneeId) {
      updates.assigneeId = req.user._id;
    }
    ...
    if (
      updates.assigneeId &&
      updates.assigneeId !== task.assignee?.toString()
    ) {
      const assigneeUser = await User.findById(updates.assigneeId);
      if (!assigneeUser) {
        throw new AppError("Assignee not found", 404);
      }
      changes.assignee = {
