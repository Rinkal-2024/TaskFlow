...
export const errorHandler = (
  ...
  if (error instanceof AppError) {
    ...
    if (
      ...
      // This is likely a validation error from express-validator
      const messages = error.message.split(", ");
      message = messages[0]; // Use the first message as the main message
  ...
  res.status(statusCode).json({
    success: false,
    message,
    error: message, // Add this for the client to access
